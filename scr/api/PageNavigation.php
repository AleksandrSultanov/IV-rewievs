...
class PageNavigation {
     ...
     * @return string|null
     */
    public function getLinks(int $all, int $limit, int $start, int $linkLimit = 10, string $varName = 'start'): ?string {
        ...
        // индекс нужного в данный момент чанка
        ...
        if ($start > 1) {
            $htmlOut .= '<li><a href="' . $link . '&' . $varName . '=0">' . $this->startChar . '</a></li>' .
                    '<li><a href="' . $link . '&' . $varName . '=' . ($start - $limit) . '">' . $this->prevChar
                    . '</a></li>';
        } else {
        ...
        foreach ($allPages[$needChunk] as $pageNum => $offset) {
            ...
            $htmlOut .= '<li><a href="' . $link . '&' . $varName . '=' . $offset . '">' . $pageNum . '</a></li>';
        }

        // Формируем ссылки "следующая", "в конец" ------------------------------------------------

        if (($all - $limit) > $start) {
            $array = array_pop($allPages);
            $htmlOut .= '<li><a href="' . $link . '&' . $varName . '=' . ($start + $limit) . '">' . $this->nextChar
                    . '</a></li>' .
                    '<li><a href="' . $link . '&' . $varName . '=' . array_pop($array) . '">'
                    . $this->endChar . '</a></li>';
        } else {
     ...
     */
    protected function searchPage(array $pagesList, int $needPage): int {
